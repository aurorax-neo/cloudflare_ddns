...
class DDNS:
    ...
    def update_dns(self, i):
        try:
            ...
            with DDNS._LOCK:
                logger.warning(f'{i.get("dns_name")} - 获取本机{i.get("ip_type", "IP")}地址失败')
            return 1
        # 获取缓存
        ...
        if record is None:
            with DDNS._LOCK:
                logger.warning('{} {}'.format(i.get('dns_name'), '未找到域名记录'))
            return 2
        ...
        try:
            if RETRY_CALLBACK(self._update_dns_record, _record_id=record.get('id', 'None'),
                ...
                with DDNS._LOCK:
                    logger.info('{} - {} {}'.format(i.get('dns_name'), ip, '记录更新成功'))
                return 4
        except Exception as e:
            with DDNS._LOCK:
                logger.error('{} - {} {} {}'.format(i.get('dns_name'), ip, '记录更新失败', e))
            return 5
    ...
    def run(cls):
        ...
        try:
            # 初始化提示
            logger.info('{} {}'.format('CFDDNS', '服务正在初始化, 请稍后...'))
            # 初始化缓存
            cls.update_cache_task()
            # 创建定时更新缓存任务
            DDNS._SCHEDULER.add_job(cls.update_cache_task, 'interval', seconds=30, max_instances=DDNS._MAX_WORKERS)
            logger.info('{} {}'.format('CFDDNS', '服务初始化完成'))
        except Exception as e:
            logger.error('{} {}'.format('CFDDNS', '缓存初始化失败', e))
            sys.exit(1)

        try:
            # 启动定时任务
            cls._SCHEDULER.start()
            logger.info("CFDDNS 服务已启动")
            while True:
                time.sleep(60)
        except (KeyboardInterrupt, SystemExit):
            cls._SCHEDULER.shutdown()
            logger.info("CFDDNS 服务已停止")
    ...
    def get_ipv6():
        try:
            ...
            if resp.status_code == 200:
                ...
                err_str = resp.text.replace('\n', '').replace('\r', '').strip()
                logger.debug('{} {}'.format('get_ipv6', err_str))
                raise Exception(err_str)
    ...
    def get_ipv4():
        try:
            ...
            if resp.status_code == 200:
                ...
                err_str = resp.text.replace('\n', '').replace('\r', '').strip()
                logger.debug('{} {}'.format('get_ipv4', err_str))
                raise Exception(err_str)
        except Exception as e_:
            raise e_
